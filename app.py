...
with tabs[0]:
    ...
    st.caption("Click directly into any cell to edit. Add new rows at the bottom. CSV is optional.")

    if mode == "Demo Portfolio":
        st.info("Demo Portfolio is read-only here. Switch to Private Session Portfolio to edit your own rows.")
        st.dataframe(
            DEMO,
        ...
        if st.button("Copy Demo to Private Portfolio", type="primary"):
            seed_private_demo()
            st.success("Demo copied. Switch to Private Session Portfolio to edit.")
    else:
        c1, c2, c3 = st.columns([1, 1, 2])
        with c1:
            if st.button("Load Demo Rows", use_container_width=True):
                seed_private_demo()
                st.rerun()
        with c2:
        ...
        st.markdown('<div class="editor-help">INLINE EDITOR: Click cells to edit. Use decimal sell percentages: 0.25 = 25%. Add rows with the + row at the bottom.</div>', unsafe_allow_html=True)

        editable_df = safe_portfolio_df(st.session_state.portfolio.copy())
        ...
        edited = st.data_editor(
            ...
            use_container_width=True,
            hide_index=True,
            key="inline_position_editor",
            column_config={
                "Coin": st.column_config.TextColumn("Coin", help="Ticker symbol, e.g. LINK"),
                "Amount": st.column_config.NumberColumn("Amount", min_value=0.0, format="%.8f"),
                "Level 1 Price": st.column_config.NumberColumn("Level 1 Price", min_value=0.0, format="$%.8f"),
                "Level 1 Sell %": st.column_config.NumberColumn("Level 1 Sell %", min_value=0.0, max_value=1.0, format="%.2f"),
                "Level 2 Price": st.column_config.NumberColumn("Level 2 Price", min_value=0.0, format="$%.8f"),
                "Level 2 Sell %": st.column_config.NumberColumn("Level 2 Sell %", min_value=0.0, max_value=1.0, format="%.2f"),
                "Level 3 Price": st.column_config.NumberColumn("Level 3 Price", min_value=0.0, format="$%.8f"),
                "Level 3 Sell %": st.column_config.NumberColumn("Level 3 Sell %", min_value=0.0, max_value=1.0, format="%.2f"),
            }
        )

        st.session_state.portfolio = safe_portfolio_df(edited)
