...
st.set_page_config(page_title="SatsCommander Crash-Locked Terminal", layout="wide")

st.markdown("""
<style>
.stApp{background:#04070D;color:#E5E7EB}.block-container{max-width:1740px;padding-top:.55rem;padding-left:1rem;padding-right:1rem}
...
.terminal-shell{border:1px solid #163252;background:linear-gradient(180deg,#0B1220,#060A12);border-radius:14px;padding:12px 16px;margin-bottom:10px}
.topbar{display:flex;justify-content:space-between;align-items:center;gap:14px}
.brand-title{font-size:29px;line-height:1;font-weight:950;color:white;letter-spacing:.2px}
...
/* no-spreadsheet position editor */
.edit-card {
    background: linear-gradient(180deg,#0B1220,#070B13);
    border: 1px solid #1E3A5F;
    border-radius: 12px;
...
/* demo preview table + start button polish */
.demo-table {
    border: 1px solid #1E3A5F;
    border-radius: 12px;
    overflow: hidden;
...
.demo-head, .demo-row {
    display: grid;
    grid-template-columns: .8fr 1fr 1fr .8fr 1fr .8fr 1fr .8fr;
    align-items: center;
}
.demo-head {
    background: #0A101A;
...
.demo-head div, .demo-row div {
    padding: 10px 12px;
    border-right: 1px solid #14253B;
}
.demo-row {
    color: #E5E7EB;
...
div.stButton > button[kind="primary"] {
    background: linear-gradient(90deg,#F7931A,#FFB347) !important;
    color: #05080F !important;
    border: 1px solid #F7931A !important;
    box-shadow: 0 0 20px rgba(247,147,26,.18) !important;
}
div.stButton > button[kind="primary"]:hover {
    filter: brightness(1.05);
    box-shadow: 0 0 26px rgba(247,147,26,.28) !important;
}


/* compact trading row portfolio preview */
.trade-preview {
    border: 1px solid #1E3A5F;
    border-radius: 12px;
    overflow: hidden;
...
.trade-row {
    display: grid;
    grid-template-columns: .75fr .85fr 1.35fr 1.35fr 1.35fr;
    align-items: center;
    border-top: 1px solid #14253B;
    background: #08101D;
    min-height: 54px;
...
.compact-action button {
    ...
    border: 1px solid #F7931A !important;
    box-shadow: 0 0 14px rgba(247,147,26,.10) !important;
    font-size: 12px !important;
}
.compact-action button:hover {
    background: rgba(247,147,26,.08) !important;
    box-shadow: 0 0 22px rgba(247,147,26,.22) !important;
}
...
for k,v in {"portfolio":pd.DataFrame(columns=REQUIRED),"executions":[],"dismissed_alerts":[],"missed_snapshots":[],"feed_log":[],"btc_total":0.0}.items():
...
def calculate(df,btc_price,prices,changes,manual_prices,executed_keys,dismissed_keys):
    ...
    if not rows:
        return pd.DataFrame(columns=[
            "Coin","Amount","Current Price","24h %","Value","Next Level","Next Level Price",
            "Progress","Alert","Command","Sell Action","Confidence","Gross Sell","BTC Gain",
            "Missed BTC","Alert Key","Executed","Dismissed","Priority"
        ])
...
def safe_portfolio_df(df=None):
    """Return a clean editable portfolio dataframe with required columns."""
    if df is None:
        df = pd.DataFrame(columns=REQUIRED)
    return normalize(df)

def seed_private_demo():
    st.session_state.portfolio = DEMO.copy()

def render_demo_preview(df):
    """Compact professional trading-row preview."""
    df = normalize(df)
    rows = []
    for _, r in df.iterrows():
        ...
        <div class="trade-row">
            <div>
                <div class="trade-coin">{r['Coin']}</div>
            </div>
            <div>
                <div class="trade-label">Amount</div>
                <div class="trade-value">{float(r['Amount']):,.4f}</div>
            </div>
            <div>
                <div class="trade-label">Level 1</div>
                <div><span class="trade-level">${float(r['Level 1 Price']):,.6f}</span> <span class="trade-sell">Sell {float(r['Level 1 Sell %']):.0%}</span></div>
            </div>
            <div>
                <div class="trade-label">Level 2</div>
                <div><span class="trade-level">${float(r['Level 2 Price']):,.6f}</span> <span class="trade-sell">Sell {float(r['Level 2 Sell %']):.0%}</span></div>
            </div>
            <div>
                <div class="trade-label">Level 3</div>
                <div><span class="trade-level">${float(r['Level 3 Price']):,.6f}</span> <span class="trade-sell">Sell {float(r['Level 3 Sell %']):.0%}</span></div>
            </div>
        </div>
        """)
    st.markdown(f"""
    <div class="mode-pill demo">● Demo Mode · Read Only</div>
...
for _col, _default in {
    "Coin":"", "Amount":0.0, "Current Price":0.0, "24h %":0.0, "Value":0.0,
    "Next Level":"", "Next Level Price":0.0, "Progress":0.0, "Alert":"STANDBY",
    "Command":"STANDBY — NO ACTION", "Sell Action":0.0, "Confidence":0,
    "Gross Sell":0.0, "BTC Gain":0.0, "Missed BTC":0.0, "Alert Key":"",
    "Executed":False, "Dismissed":False, "Priority":0
}.items():
    if _col not in results.columns:
        results[_col] = _default
...
st.markdown("""<div class="terminal-shell topbar"><div><div class="brand-title">SatsCommander</div><div class="brand-sub">Convert Alt Gains → BTC with Discipline</div></div><div class="status-pill online">● MARKET DATA LIVE</div></div>""", unsafe_allow_html=True)
...
m1.metric("BTC Progress",f"{current_stack:.8f}",f"{goal_progress:.1%}")
m2.metric("BTC Ready to Capture",f"{queued_btc:.8f}")
m3.metric("Missed Opportunities",f"{missed_btc:.8f}")
m4.metric("Discipline Score",f"{score}%")
m5.metric("Portfolio Value",f"${total_value:,.0f}")
...
with left:
    ...
    if queue.empty: st.markdown('<div class="queue-item small">No active signals. Waiting for price triggers.</div>', unsafe_allow_html=True)
    ...
    if executable.empty: st.info("No actions available yet. Signals will appear here.")
    else:
...
with center:
    st.markdown('<div class="panel-title">Active Signals</div>', unsafe_allow_html=True)
    rows_html=[]
...
with right:
    ...
    st.markdown('<div class="panel-title">Primary Signal</div>', unsafe_allow_html=True)
    if top is not None:
...
st.markdown("---")
tabs=st.tabs(["Your Portfolio","Execution History","Your Performance","Live Feed","Backup"])
with tabs[0]:
    st.header("Your Portfolio")
    st.caption("Private Mode: click any cell to edit instantly. Add new rows at the bottom. CSV is optional.")

    if mode == "Demo Portfolio":
        st.info("Demo Mode is read-only. Start editing to copy this sample into a private working portfolio.")
        render_demo_preview(DEMO)

        st.markdown('<div class="compact-action">', unsafe_allow_html=True)
        if st.button("ENTER EDIT MODE", type="secondary"):
            seed_private_demo()
            st.session_state.force_private_prompt = True
            st.success("Demo copied into Private Session Portfolio. Select Private Session Portfolio in the sidebar to edit.")
            st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
        if st.session_state.get("force_private_prompt"):
            st.markdown("""
            <div style='background:#052E1A;border:1px solid #14532D;padding:12px;border-radius:8px;color:#86EFAC;font-weight:800;'>
            ✅ Private portfolio is ready. Use the left sidebar and select <b>Private Session Portfolio</b> to edit rows directly.
            </div>
            """, unsafe_allow_html=True)
    else:
        c1, c2, c3 = st.columns([1, 1, 2])
        with c1:
            if st.button("▶ Load Sample Rows", use_container_width=True):
                seed_private_demo()
                st.rerun()
        with c2:
        ...
        st.markdown("""
        <div style='background:#07111F;border:1px solid #1E3A5F;border-radius:10px;padding:12px;margin:12px 0;color:#CBD5E1;'>
        <b>Terminal Editing Active:</b> No spreadsheet grid. Pick a coin, edit the levels, and save.
        </div>
        """, unsafe_allow_html=True)

        existing = safe_portfolio_df(st.session_state.portfolio.copy())
        existing = existing[existing["Coin"].astype(str) != ""].reset_index(drop=True)
        edit_options = ["➕ Add New Position"] + existing["Coin"].astype(str).tolist()

        st.markdown('<div class="mode-pill private">● Private Mode · Editing Enabled</div>', unsafe_allow_html=True)
        st.markdown('<div class="edit-card">', unsafe_allow_html=True)
        st.markdown('<div class="edit-card-title">Position Editor</div>', unsafe_allow_html=True)
        st.markdown('<div class="edit-card-sub">Choose an existing coin to edit, or add a new one. This keeps the app out of spreadsheet mode.</div>', unsafe_allow_html=True)

        selected_edit = st.selectbox("Position", edit_options, label_visibility="collapsed")

        if selected_edit != "➕ Add New Position" and selected_edit in existing["Coin"].values:
            row = existing[existing["Coin"] == selected_edit].iloc[0]
            default_coin = str(row["Coin"])
            default_amount = float(row["Amount"])
            default_l1 = float(row["Level 1 Price"])
            default_s1 = float(row["Level 1 Sell %"])
            default_l2 = float(row["Level 2 Price"])
            default_s2 = float(row["Level 2 Sell %"])
            default_l3 = float(row["Level 3 Price"])
            default_s3 = float(row["Level 3 Sell %"])
        else:
            default_coin = ""
            default_amount = 0.0
            default_l1 = 0.0
        ...
        with c1:
            coin = st.text_input("Coin", value=default_coin, placeholder="LINK").upper()
        with c2:
            amount = st.number_input("Amount", min_value=0.0, value=default_amount, step=1.0, format="%.8f")
        with c3:
            l1 = st.number_input("Level 1 Price", min_value=0.0, value=default_l1, step=0.01, format="%.8f")
        with c4:
            s1 = st.number_input("Level 1 Sell %", min_value=0.0, max_value=1.0, value=default_s1, step=0.05, format="%.2f")

        c5, c6, c7, c8 = st.columns([1, 1, 1, 1])
        with c5:
            l2 = st.number_input("Level 2 Price", min_value=0.0, value=default_l2, step=0.01, format="%.8f")
        with c6:
            s2 = st.number_input("Level 2 Sell %", min_value=0.0, max_value=1.0, value=default_s2, step=0.05, format="%.2f")
        with c7:
            l3 = st.number_input("Level 3 Price", min_value=0.0, value=default_l3, step=0.01, format="%.8f")
        with c8:
            s3 = st.number_input("Level 3 Sell %", min_value=0.0, max_value=1.0, value=default_s3, step=0.05, format="%.2f")

        b1, b2, b3 = st.columns([1, 1, 2])
        with b1:
            if st.button("💾 Save Position", type="primary", use_container_width=True):
                add_or_update_position(coin, amount, l1, s1, l2, s2, l3, s3)
                st.success(f"Saved {coin}.")
                st.rerun()
        with b2:
            delete_disabled = selected_edit == "➕ Add New Position"
            if st.button("🗑 Delete Position", use_container_width=True, disabled=delete_disabled):
                delete_position(selected_edit)
                st.warning(f"Deleted {selected_edit}.")
                st.rerun()

        st.markdown('</div>', unsafe_allow_html=True)

        with st.expander("Optional CSV tools"):
            uploaded = st.file_uploader("Upload CSV", type=["csv"])
            if uploaded:
                st.session_state.portfolio = safe_portfolio_df(pd.read_csv(uploaded))
                st.success("CSV loaded.")
                st.rerun()

            util_left, util_mid, util_right = st.columns([2.4, 1.0, 0.8])
            with util_right:
                st.download_button(
                    "⬇ TEMPLATE CSV",
                    pd.DataFrame(columns=REQUIRED).to_csv(index=False).encode(),
                    "satscommander_position_engine_v2_template.csv",
                    "text/csv"
                )
...
with tabs[2]:
    ...
    if not chart_df.empty and "Timestamp" in chart_df.columns:
        for _c in ["Captured BTC","Missed BTC","Discipline Score"]:
            if _c not in chart_df.columns:
                chart_df[_c] = 0
...
with tabs[3]:
    alert_df=active.reindex(columns=["Coin","Alert","Command","Current Price","Next Level Price","BTC Gain","Confidence"]).copy()
    if not alert_df.empty:
        st.dataframe(alert_df, use_container_width=True, hide_index=True)
    else:
        st.markdown("""<div style='background:#020617;border:1px solid #1E3A5F;padding:14px;border-radius:8px;color:#64748B;font-family:monospace;'>[SYSTEM] No active signals.</div>""", unsafe_allow_html=True)
with tabs[4]:
...
st.markdown('<div class="footer-note">SatsCommander Crash-Locked Terminal · data refreshed every 60 seconds · CoinGecko prices</div>', unsafe_allow_html=True)
