...
div.stButton > button, div.stDownloadButton > button {
    ...
    border: 1px solid #163252 !important;
    background: linear-gradient(180deg,#111827,#07111F) !important;
    color: #E5E7EB !important;
    box-shadow: 0 0 18px rgba(0,209,255,.06) !important;
}
div.stButton > button:hover, div.stDownloadButton > button:hover {
    border-color: #00D1FF !important;
    color: white !important;
    box-shadow: 0 0 22px rgba(0,209,255,.18) !important;
...
div.stButton > button[kind="primary"], div.stDownloadButton > button[kind="primary"] {
    ...
    box-shadow: 0 0 24px rgba(247,147,26,.22) !important;
}
div.stButton > button[kind="primary"]:hover, div.stDownloadButton > button[kind="primary"]:hover {
    filter: brightness(1.06);
    box-shadow: 0 0 30px rgba(247,147,26,.32) !important;
}
.command-note {
    font-size: 11px;
    color: #64748B;
...
div.stDownloadButton > button:hover {
    ...
    box-shadow: 0 0 18px rgba(0,209,255,.14) !important;
...
st.markdown("""<div class="terminal-shell topbar"><div><div class="brand-title">SatsCommander</div><div class="brand-sub">Pro terminal v2 · execution queue · signal insight · BTC accumulation</div></div></div>
<div style="margin-top:4px;margin-bottom:10px;"><div style="font-size:13px;color:#94A3B8;">Convert Alt Gains → BTC with Discipline</div></div><div class="terminal-shell topbar"><div><div class="brand-title">SatsCommander</div></div><div class="status-pill online">● MARKET DATA LIVE</div></div>""", unsafe_allow_html=True)
...
with left:
    ...
    if executable.empty: st.info("No actions available yet. Signals will appear here.")
        ...
        if st.button("⚡ Execute Trade",type="primary",use_container_width=True): execute_row(row,btc_price); st.rerun()
        if st.button("Ignore Signal",use_container_width=True): dismiss_row(row); st.rerun()
with center:
...
with tabs[0]:
    ...
    with util_right:
        ...
        st.markdown('<div class="utility-caption">Utility export</div>', unsafe_allow_html=True)
with tabs[1]:
    ...
    if st.button("⚠ CLEAR LOGS", type="secondary"): st.session_state.executions=[]; st.session_state.dismissed_alerts=[]; st.session_state.btc_total=0.0; st.rerun()
with tabs[2]:
...
with tabs[4]:
    ...
    bk_left, bk_mid, bk_right = st.columns([2.4, 1.0, 0.8])
    with bk_right:
        st.download_button("⬇ BACKUP JSON", json.dumps(backup_payload, indent=2).encode(), "satscommander_private_backup.json", "application/json")
        st.markdown('<div class="utility-caption">Session backup</div>', unsafe_allow_html=True)
