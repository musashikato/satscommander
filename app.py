...
def render_demo_preview(df):
    """Safe native Streamlit demo preview. No custom HTML, no raw HTML risk."""
    df = normalize(df)
    ...
    for _, r in df.iterrows():
        with st.container(border=True):
            top = st.columns([0.7, 1, 1, 1])
            with top[0]:
                st.markdown(f"### {r['Coin']}")
                st.caption(f"Amount: {float(r['Amount']):,.4f}")
            with top[1]:
                st.markdown("**Level 1**")
                st.caption(f"${float(r['Level 1 Price']):,.6f} · Sell {float(r['Level 1 Sell %']):.0%}")
            with top[2]:
                st.markdown("**Level 2**")
                st.caption(f"${float(r['Level 2 Price']):,.6f} · Sell {float(r['Level 2 Sell %']):.0%}")
            with top[3]:
                st.markdown("**Level 3**")
                st.caption(f"${float(r['Level 3 Price']):,.6f} · Sell {float(r['Level 3 Sell %']):.0%}")
...
with tabs[0]:
    ...
    st.caption("Use Demo Mode to preview. Switch to Private Mode to edit and save your own positions.")
    ...
    if mode == "Demo Portfolio":
        ...
        cta_col, _ = st.columns([0.22, 0.78])
        with cta_col:
            if st.button("Start Editing", type="primary", use_container_width=True):
                seed_private_demo()
