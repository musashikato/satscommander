...
def render_demo_preview(df):
    ...
    for _, r in df.iterrows():
        rows.append(f"""
        <div class="demo-row">
            <div class="demo-coin">{r['Coin']}</div>
            <div>{float(r['Amount']):,.4f}</div>
            <div>${float(r['Level 1 Price']):,.6f}</div>
            <div>{float(r['Level 1 Sell %']):.0%}</div>
            <div>${float(r['Level 2 Price']):,.6f}</div>
            <div>{float(r['Level 2 Sell %']):.0%}</div>
            <div>${float(r['Level 3 Price']):,.6f}</div>
            <div>{float(r['Level 3 Sell %']):.0%}</div>
        </div>
        """)
    st.markdown(f"""
    <div class="demo-table">
        <div class="demo-head">
            <div>Coin</div><div>Amount</div><div>Level 1</div><div>Sell 1</div>
            <div>Level 2</div><div>Sell 2</div><div>Level 3</div><div>Sell 3</div>
        </div>
        {''.join(rows)}
    </div>
    """, unsafe_allow_html=True)
...
with tabs[0]:
    ...
    if mode == "Demo Portfolio":
        st.info("Demo Mode is read-only sample data. To edit instantly, click the button below — it will copy this sample into your Private Session Portfolio.")
        render_demo_preview(DEMO)

        start_col, spacer_col = st.columns([0.28, 0.72])
        with start_col:
