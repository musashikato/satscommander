...
with left:
    ...
    if executable.empty: st.info("No executable rows.")
        ...
        if st.button("⚡ EXECUTE COMMAND",type="primary",use_container_width=True): execute_row(row,btc_price); st.rerun()
        if st.button("✕ DISMISS SIGNAL",use_container_width=True): dismiss_row(row); st.rerun()
with center:
...
with tabs[0]:
    ...
    dl1, dl2, dl3 = st.columns([1.2, 1, 1.2])
    with dl2:
        st.download_button("⬇ DOWNLOAD POSITION TEMPLATE", pd.DataFrame(columns=REQUIRED).to_csv(index=False).encode(), "satscommander_position_engine_v2_template.csv", "text/csv", use_container_width=True, type="primary")
        st.markdown('<div class="utility-caption">Utility export · not an execution command</div>', unsafe_allow_html=True)
with tabs[1]:
...
with tabs[4]:
    ...
    bk1, bk2, bk3 = st.columns([1.2, 1, 1.2])
    with bk2:
        st.download_button("⬇ DOWNLOAD PRIVATE BACKUP", json.dumps(backup_payload, indent=2).encode(), "satscommander_private_backup.json", "application/json", use_container_width=True, type="primary")
        st.markdown('<div class="utility-caption">Session backup · restore later</div>', unsafe_allow_html=True)
