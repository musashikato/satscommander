...
price_mode = st.sidebar.radio(
    "Price Mode",
    ["Live CoinGecko Prices", "Manual Prices"],
    help="Live prices use CoinGecko. Manual mode is fallback if a coin is unsupported."
)
...
def fetch_prices(symbols):
    ...
    # Always include BTC for conversion.
...
def calculate(df, btc_price, live_prices=None, manual_prices=None, changes=None, executed_coins=None):
    df = normalize(df)
    ...
    for _, r in df.iterrows():
        ...
        if coin == "" or amount <= 0:
            ...
            note = "Add trigger price."
            confidence = 10
        elif coin in executed_coins:
            alert = "EXECUTED"
            command = f"{coin} ALREADY EXECUTED"
            ...
            note = f"Price is {progress:.0%} of trigger." if trigger > 0 else "No action."
            confidence = 10
    ...
    df["Alert"] = alerts
    df["Command"] = commands
    df["Status Note"] = status_notes
    df["Confidence"] = confidence_vals
    df["Gross Sell"] = gross_sells
    df["BTC Captured"] = btc_captured
    df["Missed BTC"] = missed_btc_vals
...
with tabs[0]:
    ...
    if results.empty or len(results[results["Coin"] != ""]) == 0:
        ...
        c4.metric("Captured BTC", f"{captured_total:.8f}")
        c5.metric("Missed BTC", f"{missed_total:.8f}")
        ...
        if top["Alert"] == "SELL NOW":
            st.error(
                f"{top['Command']}  |  "
                f"Price: ${top['Current Price']:,.4f} ({top['Progress to Trigger']:.0%} of trigger)  |  "
                f"Convert ${top['Gross Sell']:,.2f} → {top['BTC Captured']:.8f} BTC  |  "
                f"Confidence: {top['Confidence']}%"
            )
        elif top["Alert"] == "PREPARE":
            st.success(
                f"{top['Command']}  |  "
                f"Price is {top['Progress to Trigger']:.0%} of trigger  |  "
                f"Confidence: {top['Confidence']}%"
            )
        ...
        st.progress(min(float(top.get("Confidence", 0)) / 100, 1.0), text=f"Confidence Score: {top.get('Confidence', 0)}%")
        ...
        if st.button("EXECUTE TOP COMMAND", type="primary", disabled=(top["Alert"] != "SELL NOW")):
            ...
            st.success(
                ...
                f"New session total: {st.session_state.btc_total:.8f} BTC."
            )
            st.rerun()

        st.subheader("Command Queue")
        st.dataframe(
            results[["Coin", "Amount", "Current Price", "24h Change %", "Trigger Price", "Progress to Trigger", "Sell %", "Alert", "Command", "Confidence", "Gross Sell", "BTC Captured", "Missed BTC", "Status Note"]],
            use_container_width=True,
            hide_index=True,
            column_config={
...
with tabs[3]:
    ...
    st.write("Use these tickers for live prices. Unsupported coins can still be used in Manual Prices mode.")
