...
with tabs[0]:
    ...
    if mode == "Demo Portfolio":
        ...
        st.dataframe(
            ...
            hide_index=True,
            column_config={
                "Level 1 Sell %": st.column_config.NumberColumn(format="%.0%"),
                "Level 2 Sell %": st.column_config.NumberColumn(format="%.0%"),
                "Level 3 Sell %": st.column_config.NumberColumn(format="%.0%"),
            }
        )
        start_col, spacer_col = st.columns([0.32, 0.68])
        with start_col:
            if st.button("▶ START EDITING IN PRIVATE MODE", type="primary", use_container_width=True):
                seed_private_demo()
