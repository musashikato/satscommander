...
def render_positions_table(df):
    df = normalize(df) if df is not None else pd.DataFrame(columns=REQUIRED)
    if df.empty or len(df[df["Coin"].astype(str) != ""]) == 0:
        st.markdown("""
        <div style='background:#020617;border:1px solid #1E3A5F;border-radius:8px;padding:14px;color:#64748B;font-family:monospace;'>
        [SYSTEM] No positions loaded. Add a position below to begin.
        </div>
        """, unsafe_allow_html=True)
        return
    ...
    for _, row in df.iterrows():
        if str(row.get("Coin","")).strip() == "":
            continue
        rows.append(f"""
        <div class="dark-row">
            <div><b>{row.get('Coin','')}</b></div>
            <div>{float(row.get('Amount',0)):,.4f}</div>
            <div>${float(row.get('Level 1 Price',0)):,.6f}</div>
            <div>{float(row.get('Level 1 Sell %',0)):.0%}</div>
            <div>${float(row.get('Level 2 Price',0)):,.6f}</div>
            <div>{float(row.get('Level 2 Sell %',0)):.0%}</div>
            <div>${float(row.get('Level 3 Price',0)):,.6f}</div>
            <div>{float(row.get('Level 3 Sell %',0)):.0%}</div>
        </div>
        """)
...
def add_or_update_position(coin, amount, l1, s1, l2, s2, l3, s3):
    coin = str(coin).upper().strip()
    if not coin:
        st.warning("Enter a coin symbol first.")
        return
    df = normalize(st.session_state.portfolio.copy())
    new_row = {
        "Coin": coin,
        "Amount": float(amount),
        "Level 1 Price": float(l1),
        "Level 1 Sell %": float(s1),
        "Level 2 Price": float(l2),
        "Level 2 Sell %": float(s2),
        "Level 3 Price": float(l3),
        "Level 3 Sell %": float(s3),
    }
    if coin in df["Coin"].values:
        for k, v in new_row.items():
            df.loc[df["Coin"] == coin, k] = v
    else:
        df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
    st.session_state.portfolio = normalize(df)

def delete_position(coin):
    coin = str(coin).upper().strip()
    df = normalize(st.session_state.portfolio.copy())
    st.session_state.portfolio = df[df["Coin"] != coin].reset_index(drop=True)
...
with tabs[0]:
    ...
    st.caption("Add and edit positions directly. CSV is now optional.")

    if mode=="Demo Portfolio":
        st.info("Demo Portfolio is read-only. Switch to Private Session Portfolio to edit your own positions.")
        render_positions_table(DEMO)
    else:
        # Load demo into private session for faster onboarding
        col_demo, col_clear = st.columns([1, 1])
        with col_demo:
            if st.button("Load Demo as My Portfolio", use_container_width=True):
                st.session_state.portfolio = DEMO.copy()
                st.rerun()
        with col_clear:
            if st.button("Clear My Portfolio", use_container_width=True):
                st.session_state.portfolio = pd.DataFrame(columns=REQUIRED)
                st.rerun()

        render_positions_table(st.session_state.portfolio)

        st.markdown('<div class="position-editor-card">', unsafe_allow_html=True)
        st.subheader("Add / Edit Position")
        st.markdown('<div class="editor-note">Enter a ticker, amount, and three staged sell levels. If the coin already exists, this updates that row.</div>', unsafe_allow_html=True)

        c1, c2, c3, c4 = st.columns([.75, 1, 1, 1])
        with c1:
            coin = st.text_input("Coin", placeholder="LINK").upper()
        with c2:
            amount = st.number_input("Amount", min_value=0.0, value=0.0, step=1.0, format="%.8f")
        with c3:
            l1 = st.number_input("Level 1 Price", min_value=0.0, value=0.0, step=0.01, format="%.8f")
        with c4:
            s1 = st.number_input("Level 1 Sell %", min_value=0.0, max_value=1.0, value=0.25, step=0.05, format="%.2f")

        c5, c6, c7, c8 = st.columns([1, 1, 1, 1])
        with c5:
            l2 = st.number_input("Level 2 Price", min_value=0.0, value=0.0, step=0.01, format="%.8f")
        with c6:
            s2 = st.number_input("Level 2 Sell %", min_value=0.0, max_value=1.0, value=0.35, step=0.05, format="%.2f")
        with c7:
            l3 = st.number_input("Level 3 Price", min_value=0.0, value=0.0, step=0.01, format="%.8f")
        with c8:
            s3 = st.number_input("Level 3 Sell %", min_value=0.0, max_value=1.0, value=0.40, step=0.05, format="%.2f")
        ...
        with b1:
            if st.button("➕ Save Position", type="primary", use_container_width=True):
                ...
                st.rerun()
        with b2:
            existing = normalize(st.session_state.portfolio.copy())
            options = existing["Coin"].dropna().astype(str).tolist()
            delete_coin_choice = st.selectbox("Delete", options if options else ["No positions"], label_visibility="collapsed")
            if st.button("🗑 Delete", use_container_width=True, disabled=(not options)):
                delete_position(delete_coin_choice)
                st.warning(f"Deleted {delete_coin_choice}.")
                st.rerun()

        st.markdown('</div>', unsafe_allow_html=True)

        with st.expander("Optional: upload CSV"):
    ...
    with util_right:
        st.download_button("⬇ TEMPLATE CSV", pd.DataFrame(columns=REQUIRED).to_csv(index=False).encode(), "satscommander_position_engine_v2_template.csv", "text/csv")
        st.markdown('<div class="utility-caption">Optional utility export</div>', unsafe_allow_html=True)
with tabs[1]:
